class ApplicationController < ActionController::Base
  ...
  before_action :authenticate_user!, :except => [:after_sign_in_path_for, :after_inactive_sign_up_path_for, :after_sign_up_path_for, :raw_print, :raw_single_print]
  ...
  def article_inspect(articles)
    ...
    for article in articles
      ...
      Upload.where(uploadable_type: "Article", uploadable_id: article.id).group_by(&:attachment_type).each do |k, v|
        ...
        for u in v
          @article_inspect_items << { 
            url: rails_blob_url(u.attachment, only_path: false), 
            title: u.title, 
            detail: u.detail 
          }
        end
